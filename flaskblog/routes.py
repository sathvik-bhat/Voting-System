...
from flaskblog.forms import RegistrationForm, LoginForm, UpdateAccountForm, PostForm, Vote_Form
...
def account():
    ...
    if form.validate_on_submit():
        ...
        current_user.voter_id = form.voter_id.data
        current_user.email = form.email.data
        db.session.commit()
        flash('Your account has been updated!', 'success')
        return redirect(url_for('account'))
    elif request.method == 'GET':
        form.voter_id.data = current_user.voter_id
...
def new_post():
    form = PostForm()
    if form.validate_on_submit():
        post = Post(title=form.title.data, c1=form.c1.data, c2=form.c2.data, c3=form.c3.data, c4=form.c4.data, c5=form.c5.data, author=current_user)
        db.session.add(post)
        db.session.commit()
        flash('Your post has been created!', 'success')
        return redirect(url_for('home'))
    return render_template('create_post.html', title='New Election',
                           form=form, legend='New Election')
...
@app.route("/post/<int:post_id>/cast_votes/", methods=['GET', 'POST'])
@login_required
def cast_votes(post_id):
    post = Post.query.get_or_404(post_id)
    form=Vote_Form()
    if form.validate_on_submit():
